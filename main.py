...
class BaseGameSelectScreen(Screen):
    ...
    def load_games(self):
        ...
        # Procura pelos ficheiros JSON na pasta relativa do projeto
        ...
        if not available_files:
            self.ids.options_grid.add_widget(
                Label(text=f"Nenhum jogo base encontrado para o tema '{app.selected_theme}'"))
            self.ids.next_button.disabled = True
            return

        for file_path in available_files:
            game_name = app.get_json_metadata(file_path).get(
                'game_version', file_path.stem)
            container = BoxLayout(orientation='horizontal',
                                  size_hint_y=None, height='30dp')
            checkbox = CheckBox(size_hint_x=None, width='30dp')
            checkbox.bind(active=lambda instance, value, path=str(
                file_path): app.toggle_selection(path))
...
class ExpansionSelectScreen(Screen):
    ...
    def load_expansions(self):
        ...
        if not available_files:
            self.ids.options_grid.add_widget(
                Label(text="Nenhuma expansão encontrada."))
            return
...
class MainGameScreen(Screen):
    ...
    def setup_game_session(self, selected_files):
        try:
            ...
            game_names = [Path(p).stem.replace('_', ' ').title()
    ...
    def draw_spawn(self):
        ...
        if result_type == 'NORMAL_SPAWN':
            ...
            if os.path.exists(threat_image):
                ...
                overlay_text = display_name.upper()
                print(
                    f"Warning: Threat image not found. Expected: '{threat_image}'")
            ...
            if os.path.exists(event_image_path):
                ...
                print(
                    f"Warning: Generic event image not found: '{event_image_path}'")
    ...
    def setup_danger_selectors(self):
        danger_levels = {'Blue': 'blue', 'Yellow': 'yellow',
                         'Orange': 'orange', 'Red': 'red'}
        container = self.ids.danger_selectors_container
        container.clear_widgets()
        for text, value in danger_levels.items():
            item = DangerSelector()
            checkbox = CheckBox(group='danger_level', active=(
                value == 'blue'), size_hint_x=None, width=24)
            checkbox.bind(active=lambda instance, is_active,
                          lvl=value: self.set_danger_level(lvl) if is_active else None)
...
class SpawnerApp(App):
    ...
    def on_start(self):
        self.selected_theme = ""
        self.selected_files = []
        # A lógica de copiar ficheiros já não é necessária para a "instalação padrão"
    ...
    def toggle_selection(self, path):
        ...
        if self.sm.current == 'base_game_select':
            self.sm.get_screen(
                'base_game_select').ids.next_button.disabled = not self.selected_files
